...
//        let path = createSubviewBezierPath(firstView, firstView.avatarPhotoButton, firstView.zoomButton, secondView.startTrackerButton)
    ...
    private func createSubviewBezierPath2(_ superView: FooterControlView,
        ...
        // segment 1: line
        path.addLine(to: CGPoint(x: superView.avatarPhotoButton.frame.midX,
                                 y: superView.avatarPhotoButton.frame.origin.y + superView.avatarPhotoButton.frame.height / 2 + spacing))
        // segment 2: arc
        /*path.addArc(withCenter: CGPoint(x: superView.avatarPhotoButton.frame.midX,
                                        y: superView.avatarPhotoButton.frame.midY), // center point of circle
                    radius: superView.avatarPhotoButton.frame.height / 2 + spacing, // this will make it meet our path line
                    startAngle: CGFloat(3 * (CGFloat.pi / 2)), // 270 degrees = straight up
                    endAngle: CGFloat(0), // 0 degrees = straight right
                    clockwise: true) // startAngle to endAngle goes in a clockwise direction
        // segment 3: arc
        ...
        // segment 4: arc
        path.addArc(withCenter: CGPoint(x: superView.zoomButton.frame.midX,
                                        y: superView.zoomButton.frame.midY),
                    radius: superView.zoomButton.frame.height / 2 + spacing,
                    startAngle: CGFloat.pi, // straight up
                    endAngle: CGFloat(0), // 0 radians = straight right
                    clockwise: true)
*/
